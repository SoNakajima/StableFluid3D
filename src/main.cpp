...
public:
    ...
    int allocate_data ()  {
        ...
        object       = (bool  *) malloc ( size*sizeof(bool ) );
    ...
    void draw_object ( void ) {
        ...
        for ( i=1; i<=M; i++ ) {
            ...
            for ( j=1; j<=N; j++ ) {
                ...
                for ( k=1; k<=O; k++ ) {
                    z = (k-0.5f)*h;
                    if(object[IX(i,j,k)]) glVertex3f(x, y, z);
    ...
    void draw_object2 ( void ) {
        ...
        for ( i=1; i<=M; i++ ) {
            ...
            for ( j=1; j<=N; j++ ) {
               ...
               if(object[IX(i,j,k)])  ofRect(x*dw, y*dh, h*dw-1, h*dh-1);
            }
        }
    ...
    void draw_velocity2 ( void ) {
        ...
        for ( i=1; i<=M; i++ ) {
            ...
            for ( j=1; j<=N; j++ ) {
                ...
                glVertex2f ( (x+u[IX(i,j,k)])*dw, (y+v[IX(i,j,k)])*dh);
            }
        }
        glEnd();
    ...
    void draw_density2 ( void ) {
        ...
        for ( i=0; i<=M; i++ ) {
            ...
            for ( j=0; j<=N; j++ ) {
                ...
                glColor3f (0, 0, d00); glVertex2f ( x*dw, y*dh );
                glColor3f (0, 0, d10); glVertex2f ( (x+h)*dw, y*dh );
                glColor3f (0, 0, d11); glVertex2f ( (x+h)*dw, (y+h)*dh );
                glColor3f (0, 0, d01); glVertex2f ( x*dw, (y+h)*dh );
                }
            }
        }
        glEnd ();
    ...
    void update_fluid() {
        ...
        vel_step ( M,N,O, u, v, w, u_prev, v_prev,w_prev, visc, dt, object );
        dens_step ( M,N,O, dens, dens_prev, u, v, w, diff, dt, object );
    }
    ...
    void setup() {
        ...
        gui.add(dt.setup("time delta", 0.9f,0.0f,5.0f));
        gui.add(diff.setup("diffuse",  0.0f,0.0f,5.0f));
        gui.add(visc.setup("viscosity", 0.00001f,0.0f,0.8f));
        gui.add(force.setup("add force",10.0f,0.0f,60.0f));
...
//        if(dvel) draw_velocity2();
//        if(dden) draw_density2();
 ...
 //       draw_object2();
    ...
    void clear_data ( ) {
        ...
        for( i = SIZE/2 -5; i < SIZE/2 +5; i++){
            for(int j = 10; j<O-8; j++){
        object[IX(14,i,j)] = true; object[IX(15,i,j)] = true; object[IX(16,i,j)] = true;
        object[IX(17,i,j)] = true; object[IX(18,i,j)] = true; object[IX(19,i,j)] = true;
            }
